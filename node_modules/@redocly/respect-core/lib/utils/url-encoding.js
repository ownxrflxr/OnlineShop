...
function toPercentEncoded(char) {
 ...
 */
export function encodeURIValue(value, allowReserved = false) {
    let encodedValue = encodeURIComponent(value).replace(/[!'()*]/g, (c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`);
    if (!allowReserved)
        return encodedValue;
...
export function buildQueryString(params) {
    return params
        .map((p) => `${encodeURIComponent(p.name)}=${encodeURIValue(p.value, p.allowReserved === true)}`)
        .join('&');
}
//# sourceMappingURL=url-encoding.js.map