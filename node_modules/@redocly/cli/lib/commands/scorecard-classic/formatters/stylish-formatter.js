...
export function printScorecardResults(problems, achievedLevel, targetLevelAchieved) {
    ...
    logger.info(white(`Found ${bold(red(totalErrors.toString()))} ${pluralize('error', totalErrors)} and ${bold(yellow(totalWarnings.toString()))} ${pluralize('warning', totalWarnings)} across ${bold(cyan(levelCount.toString()))} ${pluralize('level', levelCount)}\n`));
    ...
    for (const [level, levelProblems] of Object.entries(problemsByLevel)) {
        ...
        const locationPad = Math.max(...levelProblems.map((p) => {
...
//# sourceMappingURL=stylish-formatter.js.map