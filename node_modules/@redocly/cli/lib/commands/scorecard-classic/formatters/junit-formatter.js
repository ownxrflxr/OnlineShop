...
export function printScorecardResultsAsJunit(path, problems, achievedLevel, targetLevelAchieved) {
    ...
    logger.output(`<testsuites name="redocly scorecard-classic" tests="${problems.length}" failures="${failures}" errors="0" skipped="${skipped}">\n`);
    logger.output(`<testsuite name="scorecard-classic" tests="${problems.length}" failures="${failures}" errors="0" skipped="${skipped}">\n`);
    ...
    for (const problem of problems) {
        ...
        logger.output(`<testcase classname="${xmlEscape(level)}" name="${xmlEscape(problem.ruleId)}" file="${xmlEscape(location.file)}" line="${location.line}">\n`);
        if (problem.severity === 'warn') {
            logger.output(`<skipped message="${xmlEscape(message)}">${details}</skipped>\n`);
        }
        else {
            logger.output(`<failure message="${xmlEscape(message)}" type="${xmlEscape(problem.ruleId)}">${details}</failure>\n`);
        }
...
//# sourceMappingURL=junit-formatter.js.map