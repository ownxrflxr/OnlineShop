...
export async function fetchRemoteScorecardAndPlugins({ projectUrl, auth, isApiKey = false, verbose = false, }) {
    if (verbose) {
...
async function fetchProjectConfigBySlugs({ residency, orgSlug, projectSlug, auth, isApiKey, verbose = false, }) {
    ...
    if (projectResponse.status === 401 || projectResponse.status === 403) {
        ...
        throw new Error(`Unauthorized access to project: ${projectSlug}. Please check your credentials.`);
    }
...
//# sourceMappingURL=fetch-scorecard.js.map